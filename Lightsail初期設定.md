...
　  
*<details><summary>ユーザー名を初期設定の”ubuntu”から変更する方法（中上級者向け）</summary>*
...
でログインします。  
Current password: 
の表示が出るので、変更したユーザー名を入力します。  
その後、新しいパスワードを入力し設定してください。
...
各サーバーのインスタンス管理画面（WEB）の  
”ネットワーキング”内の”IPv4 ファイアウォール”から行います。  
...
:----|:----|:----
|カスタム | TCP | SSH接続用（先ほど設定した変更後のSSH用ポート番号） |
|カスタム | TCP | カルダノノード用（49513～65535までの任意番号） ※リレーIP制限 |
|カスタム | TCP | Grafana用（9100） ※リレーIP制限 |
|カスタム | TCP | Grafana用（12798） ※リレーIP制限 |
|カスタム | UDP | Chrony用（123) |
|Ping (ICMP) | ICMP |
...
:----|:----|:----
|カスタム | TCP | SSH接続用（先ほど設定した変更後のSSH用ポート番号） |
|カスタム | TCP | カルダノノード用（6000）|
|カスタム | TCP | Grafana用（3000) |
|カスタム | UDP | Chrony用（123) |
|Ping (ICMP) | ICMP |
